...
// Replace everything between {isEditing ? ( and )} with just the content of the else branch
content = content.replace(
  /\{isEditing \? \([\s\S]*?\) : \(\s*<div className="flex items-center justify-between gap-2 w-full">([\s\S]*?)<\/div>\s*\)\}/,
  `<div className="flex items-center justify-between gap-2 w-full">$1</div>`
);

// Also add dates below the score in the display block
content = content.replace(
  /<span className="bg-\[#0052cc\] text-white px-1.5 py-0.5">SCORE: \{scoreVal \|\| '—'\}<\/span>\s*<\/div>/,
...
// Make the container column so the dates stay below
content = content.replace(
...
content = content.replace(
  /<\/div>\s*\{\(testDateVal \|\| validDateVal\)/,
  `</div>
                </div>
                  {(testDateVal || validDateVal)`
);
...
console.log('Successfully refactored StudentDetailClient.tsx');
